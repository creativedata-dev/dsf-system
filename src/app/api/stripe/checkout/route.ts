...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!priceId) {
      ...
      if (!valor || valor <= 0) {
        return Response.json({ error: `Plano não tem preço configurado para cadência "${cadencia}". Configure precoMensal/precoAnual no plano.` }, { status: 422 })
      }
    ...
    const checkoutSession = await stripe.checkout.sessions.create({
      ...
      metadata: { tenantId: body.tenantId, planoId: body.planoId, cadencia },
      ...(isRecorrente ? {
        subscription_data: {
