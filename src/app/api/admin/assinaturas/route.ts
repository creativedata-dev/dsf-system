...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.tenantId || !body.planoId || !Object.values(StatusAssinatura).includes(body.status as StatusAssinatura)) {
      return Response.json({ error: `Dados inválidos: tenantId=${body.tenantId} planoId=${body.planoId} status=${body.status}` }, { status: 400 })
    }

    const plano = await prisma.plano.findUnique({ where: { id: body.planoId } })
    if (!plano) return Response.json({ error: `Plano não encontrado: ${body.planoId}` }, { status: 404 })
    ...
    console.error('[POST /api/admin/assinaturas]', msg)
